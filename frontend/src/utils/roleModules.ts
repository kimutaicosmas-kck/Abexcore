...
/** Mirrors backend ROLE_MODULE_ACCESS in rolePermissions.ts */
...
export function resolveRoleIdFromModules(
  ...
  if (selected.size === 0) return roles.find((r) => r.name === 'Sales Officer')?.id;

  let bestRole = roles.find((r) => r.name === 'Sales Officer') || roles[0];
  let bestScore = -1;

  for (const role of roles) {
    if (role.name === 'Super Admin' || role.name === 'Managing Director') continue;
  ...
  if (selected.has('finance')) {
    const financeRole = roles.find((r) => r.name === 'Finance Officer');
...
export function resolveDepartmentIdFromModules(
  ...
  return departments.find((d) => d.name === 'Management')?.id;
}
