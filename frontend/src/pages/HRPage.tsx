      ...
      {activeTab === 3 ? (
        <StatGrid>
          <StatCard
            ...
            value={advanceStats?.pendingApproval ?? 0}
            icon={<Clock className="h-5 w-5 text-white" />}
            color="from-amber-500 to-amber-700"
            onClick={() => { setAdvStatus('PENDING'); setAdvPage(1); }}
          />
          <StatCard
            title="Active Advances"
            value={advanceStats?.activeAdvances ?? 0}
            icon={<Wallet className="h-5 w-5 text-white" />}
            color="from-teal-500 to-teal-700"
            onClick={() => { setAdvStatus('ACTIVE'); setAdvPage(1); }}
          />
          <StatCard
            title="Outstanding"
            value={formatCurrency(advanceStats?.outstandingBalance ?? stats?.advancesOutstanding ?? 0)}
            icon={<DollarSign className="h-5 w-5 text-white" />}
            color="from-emerald-500 to-emerald-700"
          />
          <StatCard
            title="Recovered This Month"
            value={formatCurrency(advanceStats?.recoveredThisMonth ?? 0)}
            icon={<UserCheck className="h-5 w-5 text-white" />}
            color="from-indigo-500 to-indigo-700"
          />
          <StatCard
            title="Completed"
            value={advanceStats?.completedAdvances ?? 0}
            icon={<CircleCheck className="h-5 w-5 text-white" />}
            color="from-cyan-500 to-cyan-700"
            onClick={() => { setAdvStatus('COMPLETED'); setAdvPage(1); }}
          />
        </StatGrid>
      ) : (
        stats && (
